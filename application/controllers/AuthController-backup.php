...
class AuthController extends CI_Controller
{
	...
	public function action_reset_password($token)
	{
		...
		} else if ($this->input->post("re_password") == "") {

			$this->session->set_flashdata('error', 'Retype password cannot be empty');
			redirect(base_url('reset-password') . '/' . $token);
		} else if ($this->input->post("password") != $this->input->post("re_password")) {

			$this->session->set_flashdata('error', 'Password and retype password are not the same');
			redirect(base_url('reset-password') . '/' . $token);
		} else if (strlen($this->input->post("password")) < 6) {

			$this->session->set_flashdata('error', 'Password of at least 6 characters');
			redirect(base_url('reset-password') . '/' . $token);
		}
	...
	public function generate_user(){
		...
		foreach ($user as $key => $value) {
			...
			$user_data = array(
				'nib'=>'dinkes_prov_'.strtolower(str_replace(" ", "_",$value['nama_prov'])),
				'username'=>'dinkes_prov_'.strtolower(str_replace(" ", "_",$value['nama_prov'])),
				'password'=>md5('Dinkesprov123!@#'),
				'id_prov'=>$value['id_prov'],
				'id_role'=>5,
				'is_active'=>'1',
